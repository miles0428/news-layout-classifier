...
Usage:
    python -m classify.main train LEFT RIGHT [OTHER] [--out templates.npz]
    python -m classify.main batch INPUT OUTPUT [--templates FILE]
    python -m classify.main single IMAGE --templates FILE
    python -m classify.main bench LEFT RIGHT OTHER [--rounds N] [--verbose]
"""

import argparse, logging, shutil, sys, time, random
...
# Default discriminative columns (identified from template analysis)
DEFAULT_ACTIVE_COLS = [
...
def single(args):
    clf = NewsLayoutClassifier(active_cols=DEFAULT_ACTIVE_COLS)
    clf.load_templates(Path(args.templates))
...
def bench(args):
    ...
    for rnd in range(N):
        ...
        for f in test_L:  all_results.append(("left",  clf.classify(f), f))
        for f in test_R:  all_results.append(("right", clf.classify(f), f))
        for f in test_O:  all_results.append(("other", clf.classify(f), f))
...
def main():
    ...
    p.add_argument("--out"); p.add_argument("--resolution", type=int, default=128)
    ...
    p.add_argument("--move", action="store_true"); p.add_argument("--resolution", type=int, default=128)
    p.add_argument("--pixel-tol", type=float, default=0.05)
    p.add_argument("--min-match", type=float, default=0.30)
    p.add_argument("--active-cols", type=int, nargs="+", default=None)
